...
class AIEngine:
    ...
    def _prewarm_models(self):
        ...
        for model_info in self.models:
            try:
                # Use httpx for prewarming too
                import httpx
                with httpx.Client(timeout=30.0) as client:
                    self._try_model_sync(client, model_info, "You are a helper AI.", "Hello. Prewarming model for faster response.")
    ...
    def _create_message_hash(self, message: str, context: str, message_type: str) -> str:
        """Create hash for caching"""
        content = f"{message}:{context}:{message_type}"
        return hashlib.md5(content.encode()).hexdigest()
    
    def _try_model_sync(self, client: httpx.Client, model_info: dict, system_prompt: str, user_prompt: str) -> Optional[dict]:
        """Synchronous version for prewarming"""
    ...
    async def _try_model(self, model_info: dict, system_prompt: str, user_prompt: str) -> Optional[dict]:
...
{"transformed_message": "suggested response", "healing_score": 6, "sentiment": "neutral", "emotional_state": "understanding", "explanation": "what they really mean", "subtext": "their deeper feelings", "needs": ["emotional needs"], "warnings": []}"""
        ...
        for model_info in self.models:
            for user_prompt in user_prompts:
                try:
                    result = await self._try_model(model_info, system_prompt, user_prompt)
                    if result:
        ...
        return self._get_fallback_response(message, message_type)

    def _get_fallback_response(self, message: str, message_type: str) -> AIResponse:
