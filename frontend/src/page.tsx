...
export default function Home() {
  ...
  // Start instant demo with CORS fix and better error handling
  const startDemo = async () => {
    setLoading(true);
    
    console.log('🎭 Attempting to connect to:', `${API_BASE}/api/auth/demo`);
    ...
    try {
      const response = await fetch(`${API_BASE}/api/auth/demo`, {
        ...
        credentials: 'omit', // Don't send credentials for demo
      });
      ...
      if (response.ok) {
        ...
        // Show success message with demo stats
        if (data.message) {
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        ...
        alert(`🚫 Connection blocked by CORS policy!\n\nYour FastAPI server needs CORS middleware.\n\nPlease add this to your FastAPI main.py:\n\nfrom fastapi.middleware.cors import CORSMiddleware\n\napp.add_middleware(\n    CORSMiddleware,\n    allow_origins=["*"],\n    allow_credentials=True,\n    allow_methods=["*"],\n    allow_headers=["*"]\n)`);
      } else {
  ...
  // Transform message with AI and better error handling
  const transformMessage = async () => {
    if (!demoMessage.trim()) return;
    
    setLoading(true);
    console.log('🔄 Transforming message:', demoMessage);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.error('❌ Transform HTTP Error:', response.status, errorText);
      ...
      // Always provide fallback for demo experience
      ...
      <div className="container mx-auto px-4 py-16 md:py-24">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          ...
          <div className="bg-white bg-opacity-10 backdrop-blur-lg rounded-2xl p-8 text-center">
            ...
            <h3 className="text-xl font-semibent text-white mb-3">Heal</h3>
            <p className="text-purple-100">
