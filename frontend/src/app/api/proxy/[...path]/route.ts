...
const LOCAL_BACKEND = 'http://localhost:8000'; // Local development backend
...
async function handleRequest(
  ...
  // Pick backend based on environment and health
  let baseUrl: string;
  ...
  if (isDev) {
    ...
    // Production: use health check for failover
    const usePrimary = await checkPrimaryHealth();
    baseUrl = usePrimary ? PRIMARY : SECONDARY;
  }
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ path: string[] }> }) {
  ...
  if (resolvedParams.path.length === 1 && resolvedParams.path[0] === 'status') {
    ...
    let primaryHealthy: boolean;

    if (isDev) {
      currentBackend = "Local Development";
      primaryHealthy = true;
    } else {
    ...
    return NextResponse.json({
      ...
      domain: isDev ? "localhost" : "thethirdvoice.ai",
      primaryHealthy,
      currentBackend,
      lastChecked: isDev ? "N/A (local mode)" : new Date(lastChecked).toISOString(),
