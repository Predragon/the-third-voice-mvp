...
export default function TheThirdVoiceApp() {
  ...
  const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    e.target.focus(); // Ensure focus remains after state update
  };

  const handleInputInteraction = (e: React.TouchEvent<HTMLTextAreaElement> | React.FocusEvent<HTMLTextAreaElement>) => {
    // Removed translateZ(0) to prevent keyboard retraction issues on Android
    // (e.target as HTMLTextAreaElement).style.transform = 'translateZ(0)';
